...
function cdf( x, options ) {
	...
	if ( isNumber( x ) ) {
		return cdf1( x, opts.k, opts.lambda );
	}
	if ( isMatrixLike( x ) ) {
		if ( opts.copy !== false ) {
		...
		return cdf5( out, x, opts.k, opts.lambda );
	}
	if ( isTypedArrayLike( x ) ) {
		...
		return cdf6( out, x, opts.k, opts.lambda );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return cdf4( x, opts.k, opts.lambda, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return cdf3( out, x, opts.k, opts.lambda, opts.accessor );
		}
		return cdf2( out, x, opts.k, opts.lambda );
	}
	return NaN;
} // end FUNCTION cdf()
